...
import tifffile
from simulate_gradz import simulate_gradz
...
class CellposeModel(UnetModel):
    ...
    def eval(self, x, batch_size=8, channels=None, channel_axis=None, 
             ...
             loop_run=False, model_loaded=False, filename=None, do_sim_Z=False):
        """
        ...
        if (isinstance(x, list) or x.squeeze().ndim==5) and not isinstance(x[0], str):
            masks, styles, flows = [], [], []
            ...
            for i in iterator:
                maski, flowi, stylei = self.eval(x[i], 
                                                 ...
                                                 progress=progress,
                                                 loop_run=(i>0),
                                                 model_loaded=model_loaded, filename=filename, do_sim_Z=do_sim_Z)# if filename is not None else x[i].split('/')[-1])
                masks.append(maski)
                flows.append(flowi)
            ...
            masks, styles, dP, cellprob, p = self._run_cp(x, 
                                                          ...
                                                          do_3D=do_3D, 
                                                          anisotropy=anisotropy,
                                                          stitch_threshold=stitch_threshold, filename=filename, channels=channels, channel_axis=channel_axis, z_axis=z_axis, do_sim_Z=do_sim_Z
                                                          )
    ...
    def _run_cp(self, x, compute_masks=True, normalize=True, invert=False,
                ...
                interp=True, anisotropy=1.0, do_3D=False, stitch_threshold=0.0, filename=None, channels=None, channel_axis=None, z_axis=None, do_sim_Z=False
                ):
        ...
        if do_3D :
            ...
            yf, styles = self._run_3D(img, rsz=rescale, anisotropy=anisotropy, 
                                      ...
                                      tile_overlap=tile_overlap, do_sim_Z=do_sim_Z)
            ...
            if isinstance(x[0], str):
                img = imread(x[0])[np.newaxis,...]
                img = transforms.convert_image(img, channels, channel_axis=channel_axis, z_axis=z_axis,
                                            do_3D=(do_3D or stitch_threshold>0 ), 
                                            normalize=False, invert=False, nchan=self.nchan)[0]
                shape = [nimg, img.shape[0], img.shape[1], img.shape[2]]
            tqdm_out = utils.TqdmToLogger(models_logger, level=logging.INFO)
            iterator = trange(nimg, file=tqdm_out) if nimg>1 else range(nimg)
            ...
            for i in range(nimg):#iterator:
                if isinstance(x[i], str) and i > 0:
                    img = np.asarray(imread(x[i]))[np.newaxis,...]
                    img = transforms.convert_image(img, channels, channel_axis=channel_axis, z_axis=z_axis,
                                                do_3D=(do_3D or stitch_threshold>0 ), 
                                                normalize=False, invert=False, nchan=self.nchan)[0]
                elif not isinstance(x[i], str): 
                    img = np.asarray(x[i])
                if normalize or invert:
                    img = transforms.normalize_img(img, invert=invert)
                if rescale != 1.0:
                ...
                # save_fn = filename if filename else x[i]
                # np.savez_compressed('temp_zx/%s' % save_fn.split('/')[-1].replace('.tif', '.npz'), yf=yf)
                # continue
                cellprob[i] = yf[:,:,2]
            ...
            if do_3D:
                ...
                for i in iterator:
                    outputs = dynamics.compute_masks(dP[:,i], cellprob[i], niter=niter, cellprob_threshold=cellprob_threshold,
                                                         flow_threshold=flow_threshold, interp=interp, resize=resize if not do_sim_Z else None,
                                                         use_gpu=self.gpu, device=self.device)
