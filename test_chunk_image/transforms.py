...
def resize_image(img0, Ly=None, Lx=None, rsz=None, interpolation=cv2.INTER_LINEAR, no_channels=False):
    ...
    # if (img0.ndim > 2 and no_channels) or (img0.ndim == 4 and not no_channels):
    #     if no_channels:
    #         imgs = torch.zeros((img0.shape[0], Ly, Lx), dtype=torch.float32)
    #     else:
    #         imgs = torch.zeros((img0.shape[0], Ly, Lx, img0.shape[-1]), dtype=torch.float32)
    #         imgs = torch.nn.functional.interpolate(img0.permute(3,0,1,2), size=(Ly, Lx), mode='bilinear', align_corners=False).permute(1,2,3,0)
    #         # imgs = 
            
    #     # for i in range(img0.shape[0]):
    #     #     img = img0[i]
    #     #     img = img.permute(2, 0, 1)#.unsqueeze(0)
    #     #     img = torch.nn.functional.interpolate(img, size=(Ly, Lx), mode='linear', align_corners=False)
    #     #     # img = img.squeeze(0).permute(1, 2, 0)
    #     #     imgs[i] = img
    # else:
    #     img0 = img0.permute(2, 0, 1).unsqueeze(0)
    #     imgs = torch.nn.functional.interpolate(img0, size=(Ly, Lx), mode='linear', align_corners=False)
    #     imgs = imgs.squeeze(0).permute(1, 2, 0)
...
def pad_image_ND(img0, div=16, extra = 1):
    ...
    # if img0.ndim > 3:
    #     pads = torch.tensor([[0, 0], [0, 0], [xpad1, xpad2], [ypad1, ypad2]])
    # else:
    #     pads = torch.tensor([[0, 0], [xpad1, xpad2], [ypad1, ypad2]])

    # I = torch.nn.functional.pad(torch.from_numpy(img0), pads, mode='constant')

    # Ly, Lx = img0.shape[-2:]
    # ysub = torch.arange(xpad1, xpad1 + Ly)
    # xsub = torch.arange(ypad1, ypad1 + Lx)
    # return I.numpy(), ysub.numpy(), xsub.numpy()
