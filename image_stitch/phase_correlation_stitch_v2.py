...
def get_stitch_tform(
    ...
    '''
    Coarse reg by 3D PCC (separate into small chunks)
    '''
    max_shift = 0.05
    ...
    # down_r = 0.5
    zstep = 20
    ziterator = list(range(tile_overlap['0-0'][0].shape[0]//zstep))
    ziterator = [zi*zstep for zi in ziterator]
    if ziterator[-1] != tile_overlap['0-0'][0].shape[0]: ziterator[-1] = tile_overlap['0-0'][0].shape[0]
    for i in range(ncol):
        j = 0
        ...
        for j in range(1, nrow):
            ...
            for pi, pj in neighbor_row:
                if f'{i+pi}-{j+pj}' in tform_stack_coarse_colrow: 
                    ...
                    pre_shift = [0, 0, 0]
                    for zi in range(len(ziterator)-1):
                        shift, error, diffphase = phase_cross_correlation(reference_image[ziterator[zi]:ziterator[zi+1]], moving_image[ziterator[zi]:ziterator[zi+1]], overlap_ratio=0.8)
                        # shift = [s/down_r for s in shift]
                        if abs(shift[0]) > seg_shape[0]*max_shift or abs(shift[1]) > seg_shape[1]*max_shift or abs(shift[2]) > seg_shape[2]*max_shift:
                            shift = pre_shift

                        tz, tx, ty = shift
                        print(datetime.now(), f"{ziterator[zi]}:{ziterator[zi+1]} done phase cross correlation, shift: {shift}, error: {error:.6f}, dphase: {diffphase:.6f}")
            ...
            for zi in trange(moving_image[0].shape[0]-1, -1, -1, desc=f"mov ({i},{j})"):
                ...
                for pi, pj in neighbor_row:
                    if f'{i+pi}-{j+pj}' in tform_stack_refine_colrow[zi]: 
                        ...
                        if abs(shift[0]) > seg_shape[1]*max_shift or abs(shift[1]) > seg_shape[2]*max_shift:
                            shift = pre_shift
                        else:
                            shift = [s/down_r for s in shift]
