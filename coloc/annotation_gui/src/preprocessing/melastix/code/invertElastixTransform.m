function reg_params = invertElastixTransform(transformDir,outputDir)
%
...
%%%% New. Set param metric to DisplacementMagnitudePenalty
for i = 1:length(params)
    %new_param = elastix_parameter_read(params{i});
    %new_param.Metric = 'DisplacementMagnitudePenalty';
    changeParameterInElastixFile(params{i},'Metric','DisplacementMagnitudePenalty')
end
...
[reg_params,~] = elastix(fixedImage,fixedImage,outputDir,params(end),'t0',coefFiles(end));


%Force the transform chain to end here (in theory it would otherwise attempt to carry on
%and undo the inverse transform)
reg_params.TransformParameters{1}.InitialTransformParametersFileName='NoInitialTransform';
