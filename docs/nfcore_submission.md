...
The module in [`modules/nf-core/nis`](../modules/nf-core/nis) is lint-clean
(`nf-core modules lint nis` → 41/41) and prettier-formatted. The actual PR must be
...
1. **Publish the container.** Build and push the image referenced by the module.
   ...
   ```bash
   docker build -f containers/nis/Dockerfile -t quay.io/nf-core/nis:1.0.0 .
   docker push quay.io/nf-core/nis:1.0.0
   # then set `container "quay.io/nf-core/nis:1.0.0"` in modules/nf-core/nis/main.nf
   ```
...
mkdir -p modules/nf-core/nis/tests
cp "$SRC"/modules/nf-core/nis/main.nf                 modules/nf-core/nis/
cp "$SRC"/modules/nf-core/nis/meta.yml                modules/nf-core/nis/
cp "$SRC"/modules/nf-core/nis/tests/main.nf.test      modules/nf-core/nis/tests/
cp "$SRC"/modules/nf-core/nis/tests/main.nf.test.snap modules/nf-core/nis/tests/

# 3. Validate locally
nf-core modules lint nis
pre-commit run --all-files
nf-core modules test nis --profile docker   # once the container is published

# 4. Commit, push, open PR against master
git add modules/nf-core/nis
git commit -m "new module: nis"
git push -u origin nis-module
gh pr create --repo nf-core/modules --base master \
  --title "new module: nis" --body-file ../PR_BODY.md
```
...
`nis` runs whole-brain 3D nuclei instance segmentation on a single lightsheet
...
the C++/LibTorch `nis` executable from
...
- Ensure the test works with Docker / Singularity:
  - [x] `nf-core modules test nis --profile docker` <!-- stub; real GPU test pending test-data -->
  - [x] `nf-core modules test nis --profile singularity`
  - [ ] `nf-core modules test nis --profile conda` <!-- intentionally unsupported (GPU binary) -->
...
- Versions come from `eval("cat /usr/local/share/nis/VERSION")` because the binary
